...
impl<T> EntityAllocator<T> {
    ...
    fn reserve_raw(&self, count: usize) -> isize {
        ...
        loop {
            let new = current
                .checked_sub(count.try_into().unwrap_or_else(|_| too_many_entities()))
    ...
    pub fn reserve_multiple(&self, _count: usize) -> ReserveMultiple<T> {
        todo!();
    }
    ...
    pub fn flush(&mut self, mut allocate: impl FnMut(Entity) -> T) {
        ...
        self.free_list.truncate(new_slots_count);
        *self.reserve_cursor.get_mut() = new_slots_count as isize;
    }
