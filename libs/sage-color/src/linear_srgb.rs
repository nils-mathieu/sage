use crate::{Srgba, gamma_function};

/// A color, encoded in the sRGB (linear) color space, with an alpha channel.
...
impl From<Srgba> for LinearSrgba {
    fn from(color: Srgba) -> Self {
        Self {
            red: gamma_function(color.red),
            green: gamma_function(color.green),
            blue: gamma_function(color.blue),
            alpha: color.alpha,
        }
    }
}
